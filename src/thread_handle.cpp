...
namespace ftn {
  ...
  ThreadHandle::ThreadHandle()
  {
    ...
    std::thread([this](){
      while (1) {
        if (!send_msg_queue_mosquitto.empty()) {
          MessagePair pair = send_msg_queue_mosquitto.front();
          Core::instance().deliver(pair, true);
          send_msg_queue_mosquitto.pop();
        }
        else {
          send_msg_queue_mosquitto.wait();
        }
      }
    }).detach();
  ...
  Publisher ThreadHandle::_advertise(std::string& topic, const char* tname)
  {
      auto publish_f = [this, topic, tname](std::shared_ptr<Message> msg, const char* msg_tname)
      {
        ...
        send_msg_queue_mosquitto.emplace(p);
        send_msg_queue_mosquitto.notify();
      };
